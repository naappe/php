async function load(){
  ...
  state.parts=batches.flat();state.activeGen=g[0]?.id||'gen1';state.scopeGen=state.activeGen;state.selectedGen=g[0]||null;
...
function renderFleetTabs(){
  ...
  document.querySelector('#activeEvidence').innerHTML=`<span class="confidence ${cclass(g.oemEvidenceStatus)}">${esc(g.oemEvidenceStatus)}</span>`;
  ...
  document.querySelectorAll('[data-fleet]').forEach(b=>b.onclick=()=>selectMachine(b.dataset.fleet,true));
}

function overviewSvg(){
  const mark=(n,id,cx,cy,tx,ty)=>`<g style="cursor:pointer" onclick="openSystem('${id}')"><path d="M${cx} ${cy}L${tx} ${ty}" stroke="#7590a5" stroke-width="1.3"/><circle cx="${cx}" cy="${cy}" r="15" fill="#173b61" stroke="#fff" stroke-width="2"/><text x="${cx}" y="${cy+4}" text-anchor="middle" font-size="10" font-weight="900" fill="#fff">${String(n).padStart(2,'0')}</text></g>`;
  return `<svg viewBox="0 0 860 400" role="img" aria-label="Diesel engine systems orientation">${flowDefs()}<rect x="0" y="0" width="860" height="400" fill="#fbfaf6"/><path d="M110 110h130c35 0 30 38 62 38h250" stroke="#2f78b7" stroke-width="7" fill="none" marker-end="url(#blueArr)"/><path d="M518 104h112c25 0 23 36 50 36h75" stroke="#c66a3d" stroke-width="7" fill="none" marker-end="url(#orangeArr)"/><path d="M308 308h247" stroke="#b27a16" stroke-width="6" fill="none" marker-end="url(#goldArr)"/>${engineBase()}<rect x="225" y="73" width="84" height="42" fill="#eef4f7" stroke="#5e768c"/><path d="M238 86h58M238 98h58" stroke="#7791a8"/><circle cx="207" cy="147" r="24" fill="#e8f6f3" stroke="#0d857b" stroke-width="4"/><path d="M207 127c-13 12-13 27 0 40 13-13 13-28 0-40Z" fill="none" stroke="#0d857b" stroke-width="4"/><rect x="646" y="178" width="70" height="92" fill="#eef6fa" stroke="#457da5"/>${[659,673,687,701].map(x=>`<path d="M${x} 190v68" stroke="#457da5"/>`).join('')}${mark(1,'air',85,78,207,147)}${mark(2,'fuel',85,154,302,107)}${mark(3,'lube',85,230,343,305)}${mark(4,'cooling',85,306,646,220)}${mark(5,'combustion',430,52,430,145)}${mark(6,'crank',430,348,430,264)}${mark(7,'valve',775,78,455,108)}${mark(8,'start',775,154,563,278)}${mark(9,'control',775,230,584,116)}${mark(10,'service',775,306,525,236)}<text x="430" y="382" text-anchor="middle" font-size="9" font-weight="800" fill="#738596">FUNCTIONAL ORIENTATION · NOT OEM INSTALLATION GEOMETRY</text></svg>`
}

function renderMachineConsole(){
  const g=activeGenerator();if(!g)return;
  document.querySelector('#machineTitle').textContent=`${g.name} · ${g.manufacturer} ${g.model}`;
  document.querySelector('#machineSub').textContent='Select a numbered engine system to enter its technical plate, modules and related spare-part records.';
  document.querySelector('#machineModel').textContent=`${g.manufacturer} ${g.model}`;
  document.querySelector('#machineRating').textContent=`${g.ratingKw} kW`;
  document.querySelector('#machineIdentity').textContent=g.identityStatus;
  document.querySelector('#machineEvidence').textContent=g.oemEvidenceStatus;
  document.querySelector('#machineNote').textContent=g.note;
  document.querySelector('#machineFigure').innerHTML=overviewSvg();
  const row=(s,i)=>`<button class="system-link" data-machine-system="${s.id}"><span class="n">${String(i+1).padStart(2,'0')}</span><span><b>${esc(s.name)}</b><small>${esc(plateProfile(s.id).purpose)}</small></span></button>`;
  document.querySelector('#machineSystemLeft').innerHTML=state.systems.slice(0,5).map((s,i)=>row(s,i)).join('');
  document.querySelector('#machineSystemRight').innerHTML=state.systems.slice(5).map((s,i)=>row(s,i+5)).join('');
  document.querySelectorAll('[data-machine-system]').forEach(b=>b.onclick=()=>{state.scopeGen=g.id;openSystem(b.dataset.machineSystem)});
...
function selectMachine(id,goHome=false){
  const g=state.generators.find(x=>x.id===id);if(!g)return;
  state.activeGen=id;state.scopeGen=id;state.selectedGen=g;renderFleetTabs();renderMachineConsole();renderScope();renderInfographics();
...
function renderAll(){
  ...
  renderScope();renderInfographics();
...
function renderParts(){if(!state.parts.length)return;const q=document.querySelector('#partSearch').value.toLowerCase().trim(),g=document.querySelector('#genFilter').value,s=document.querySelector('#systemFilter').value,c=document.querySelector('#confidenceFilter').value;let rows=state.parts.filter(p=>{const hay=(p.id+' '+p.d+' '+(p.p||[]).join(' ')+' '+p.s+' '+p.m).toLowerCase();if(q&&!hay.includes(q))return false;if(g!=='all'&&!(p.g||[]).includes(g))return false;if(s!=='all'&&p.s!==s)return false;if(c!=='all'&&p.c!==c)return false;return true});rows.sort((a,b)=>a.s.localeCompare(b.s)||a.d.localeCompare(b.d));document.querySelector('#resultText').textContent=`Showing ${rows.length} of ${state.parts.length} engine spare-part records`;document.querySelector('#partRows').innerHTML=rows.length?rows.map(p=>`<div class="part-row" data-part="${esc(p.id)}"><div class="part-id">${esc(p.id)}</div><div class="part-desc">${esc(p.d)}<small>${p.p?.length?'P/N '+esc(p.p.join(' / ')):'No part number detected'}</small></div><div>${esc(p.s)}<div class="source">${esc(p.m)}</div></div><div><span class="confidence ${cclass(p.c)}">${esc(p.c)}</span></div><div class="genchips">${p.g?.length?p.g.map(x=>`<span class="genchip">${esc(genName(x))}</span>`).join(''):'<span class="source">Unassigned</span>'}</div></div>`).join(''):'<div class="empty">No spare parts match these filters.</div>';document.querySelectorAll('[data-part]').forEach(el=>el.onclick=()=>openPart(el.dataset.part))}

function openPart(id){const p=state.parts.find(x=>x.id===id),sys=state.systems.find(s=>s.name===p.s);const gens=p.g?.length?p.g.map(genName).join('<br>'):'No generator mapping yet';openDrawer(`<div class="eyebrow">PART RECORD · ${esc(p.id)}</div><h3>${esc(p.d)}</h3><span class="confidence ${cclass(p.c)}">${esc(p.c)}</span>${sys?`<div class="drawer-grid"><div class="drawer-plate">${plateSvg(sys,true,p.m)}</div><div class="drawer-legend">${legendHtml(sys)}</div></div>`:''}<div class="mini"><div><small>PART NUMBER</small><b>${p.p?.length?esc(p.p.join(' / ')):'Not detected'}</b></div><div><small>SYSTEM</small><b>${esc(p.s)}</b></div><div><small>MODULE</small><b>${esc(p.m)}</b></div><div><small>ENGINE APPLICATION</small><b>${gens}</b></div></div><div class="learning-note"><b>Evidence state:</b> ${esc(confMeaning[p.c]||'Evidence state not defined.')} Stock quantity is intentionally excluded from this training interface.</div>`)}

function openDiagnostic(spn){const d=diagnostic.find(x=>x.spn===spn),s=state.systems.find(x=>x.id===d.system);openDrawer(`<div class="eyebrow">FAULT PATH · SPN ${esc(d.spn)}</div><h3>${esc(d.name)}</h3>${s?`<div class="drawer-grid"><div class="drawer-plate">${plateSvg(s,true)}</div><div class="drawer-legend">${legendHtml(s)}</div></div>`:''}<div class="mini-flow">${d.nodes.map((n,i)=>`${i?'<span class="arrow">→</span>':''}<span class="node">${esc(n)}</span>`).join('')}</div><div class="learning-note" style="margin-top:12px">${esc(d.note)}</div>`)}
function openMaintenance(i){const m=maintenance[i],s=state.systems.find(x=>x.id===m.system);openDrawer(`<div class="eyebrow">MAINTENANCE RELATIONSHIP</div><h3>${esc(m.title)}</h3>${s?`<div class="drawer-grid"><div class="drawer-plate">${plateSvg(s,true)}</div><div class="drawer-legend">${legendHtml(s)}</div></div>`:''}<div class="module-list">${m.items.map((x,i)=>`<div class="module"><span class="module-icon">${i+1}</span><span>${esc(x)}</span></div>`).join('')}</div><div class="learning-note" style="margin-top:12px">Intervals, torque values, clearances and consumable specifications must come from the correct engine-specific service literature.</div>`)}
...
document.querySelector('#globalSearch').addEventListener('keydown',e=>{if(e.key!=='Enter')return;const q=e.target.value.trim();if(!q)return;const gen=state.generators.find(g=>(g.name+' '+g.manufacturer+' '+g.model).toLowerCase().includes(q.toLowerCase()));if(gen){openGenerator(gen.id);return}const sys=state.systems.find(s=>(s.name+' '+s.modules.join(' ')).toLowerCase().includes(q.toLowerCase()));if(sys){showPage('systems');setTimeout(()=>openSystem(sys.id),60);return}showPage('parts');document.querySelector('#partSearch').value=q;renderParts()});

load().catch(err=>document.querySelector('.content').insertAdjacentHTML('afterbegin',`<div class="warningline"><b>DATA ERROR</b><span>${esc(err.message)}</span></div>`));